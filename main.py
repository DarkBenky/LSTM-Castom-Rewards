...
def create_lstm_model_Price():
    ...
    model.add(Dropout(DROPOUT))
    model.add(Dense(1, activation='sigmoid'))  # Predict the future price
    
    return model

def create_dense_model_Price():
    model = Sequential()
    
    # First Dense layer
    model.add(Dense(NEURONS, activation='tanh', input_shape=(NUM_VALUES, data_len)))
    model.add(Dropout(DROPOUT))
...
def train_and_evaluate_price_model(model, X_price, y_price, model_name):
    ...
    def calculate_profits(predictions, actual_prices):
        ...
        profits = actual_prices - predictions[0]
        for i in range(len(profits)):
            wandb.log({f"profits_{model_name}": profits[i]})
            wandb.log({f"actual_price_{model_name}":actual_prices[i]})
...
lstm_model_price = create_dense_model_Price()
train_and_evaluate_price_model(lstm_model_price, X_price, y_price, "dense_model_price")